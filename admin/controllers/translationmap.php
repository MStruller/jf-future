...
*/
	...
	function save()
	{
		...
		if($success = $translationmap->generateTranslationMapManual($reference_id,$translation_id,$contentElement->getTableName(),$lang_code))
		{
	...
	function edit()
	{
		...
		*/
		...
		$tranFilters = getTranslationFilters($this->_catid, $contentElement);
		...
		$requestFilters = JRequest::getVar('filters');
		if(count($requestFilters))
		{
			foreach($requestFilters as $key => $value)
			{
				$filters[$key] = 'c.'.$key.'=\''.$value.'\'';
			}
		}
		$this->view->assignRef('requestFilters', $requestFilters);
		
		$order = JFactory::getApplication()->getUserStateFromRequest("com_joomfish.view.translationmap.filter_order", 'filter_order', null);
		$orderDir = JFactory::getApplication()->getUserStateFromRequest("com_joomfish.view.translationmap.filter_order_Dir", 'filter_order_Dir', 'ASC');
		//FB::dump($filters);
		$filterHTML = array();
		foreach ($tranFilters as $tranFilter)
		{
			...
			{
				if(!in_array($tranFilter->filterType,array_keys($filters)) && $tranFilter->filterType !='language')
				{
					$filters[$tranFilter->filterType] = $tranFilter->createFilter($contentElement);
				//FB::dump($tranFilter,'tran');
				
				
				//FB::dump($tranFilter->filterType,'tran');
		...
		$contentmaprows = $translationmap->getContentMap($contentElement,$this->_language_id,$filters,$order,$orderDir);
		...
		$lang = $this->_joomfishManager->getLanguageByID($this->_language_id);
		$this->view->assignRef('language',$lang->code);
