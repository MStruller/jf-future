...
class TranslateForms
{
	...
	function setForms($translation_id = null, $contentid = null, $where = false)
	{
		...
		//FB::dump($this->trans_form);
		//we need to set special fields
		...
		/*
		here we call the child for special?
		*/
		if(method_exists ($this,'setFields'))
		{
			$this->setFields($jfFields,$formFields);
		}
		//FB::dump($this->orig_form);
		//FB::dump($this,'t');
		/*
		*/
		if($contentid)
	...
	{
		//TODO origin to left and copy button to left trans to right and delete button to right
		/*
		$html = '';
		...
		*/
		...
		if(strtolower($field->__get('type')) != 'spacer')
		{
			...
			$htmlDiv1 = '<div class="'.$class.'parambuttons">';
			$htmlDiv2 = '</div>';

			//copy button
			$htmlLeft .= $htmlDiv1;
				$htmlLeft .= '<a class="'.$class.'toolbar" onclick="'.$onclickCopy.'">';
					$htmlLeft .= '<span class="icon-32-copy">';
					$htmlLeft .= '</span>';
					//JText::_( 'COPY' );
				$htmlLeft .= '</a>';
			$htmlLeft .= $htmlDiv2;
			//delete button
			$htmlRight .= $htmlDiv1;
				$htmlRight .= '<a class="'.$class.'toolbar" onclick="'.$onclickDelete.'">';
					$htmlRight .= '<span class="icon-32-delete">';
					$htmlRight .= '</span>';
					//JText::_( 'DELETE' ); 
				$htmlRight .= '</a>';
			$htmlRight .= $htmlDiv2;
		...
		$html = array('left'=>$htmlLeft,'right'=>$htmlRight);
		return $html;
	}
						...
						<?php foreach ($this->trans_form->getFieldset($name) as $field) : ?>
							...
							<li>
								<div class="width-100 fltlft">
									<?php $buttons = $this->getButtons($this->orig_form->getField($field->__get('fieldname'),$field->__get('group'))); ?>
									...
									<div class="width-10 jf_parambuttons fltlft">
										<?php echo $buttons['left']; //echo $this->getButtons($this->orig_form->getField($field->__get('fieldname'),$field->__get('group'))); ?>
									</div>
		...
		?>
			<div class="width-100 fltlft">
				...
				<div class="width-10 jf_parambuttons fltlft">
					...
					<?php $buttonsOutput = $this->getButtons($this->orig_form->getField($field->__get('fieldname'),$field->__get('group')),$buttons); ?>
					<?php echo $buttonsOutput['left'];//echo $this->getButtons($this->orig_form->getField($field->__get('fieldname'),$field->__get('group')),$buttons); ?>
					<?php else : ?>
					<?php echo '&nbsp;'; ?>
					<?php endif; ?>
