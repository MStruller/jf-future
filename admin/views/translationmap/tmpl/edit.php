...
function save()
{
	...
	var url = 'index.php?option=com_joomfish&task=translationmap.save&reference_id='+ajax_reference_id.value+'&translation_id='+ajax_translation_id.value+'&language_id='+ajax_language_id.value+'&<?php echo JUtility::getToken(); ?>=1';
	var myRequest = new Request({
		...
		onSuccess: function(responseText,responseXML){
			...
			else
			{
				...
				//is other than joomfish
				//TODO remove the tanslationMapButton
				window.parent.parent.removeTransmapButton();
				window.parent.parent.SqueezeBox.close()
				//is joomfish window.parent.document.adminForm.submit();
...
function action(action,action_id,content_id_reference,id,lang_id,lang_name) {
	var actiondiv = document.id('action');
	...
	{
		actiondiv.set('html','action set as translation, reference_id:'+content_id_reference+'</br>translation_id:'+id + ', language_id:'+lang_id + ', language_name:'+lang_name);
	...
	{
		actiondiv.set('html','action set as reference, reference_id:'+id+'</br>translation_id:'+content_id_reference + ', language_id:'+lang_id + ', language_name:'+lang_name);
...
<div style="height:450px;">
	...
	$filterOptions .= '<td align="center" nowrap="nowrap">';
	$filterOptions .= JText::_('ORG_LANGUAGE');
	$filterOptions .= '<br>';
	$filterOptions .= '<select name="filter_language" class="inputbox" onchange="this.form.submit()">';
				$filterOptions .= '<option value="">'.JText::_('JOPTION_SELECT_LANGUAGE').'</option>';
				...
				$filterOptions .= JHtml::_('select.options', $langs, 'value', 'text', $this->filter_language);
...
?>
		...
		<ul class="adminformlist" style="overflow-y:scroll;height:253px;">
			...
			<li class="<?php echo "li_row$k list"; ?>">
				...
				<div class="width-15 fltlft" style="width:15%;">
					<?php echo ($row->reference_id ? $row->reference_id : '&nbsp;'.($row->orig_language != '*' ? '<a class="hasTip original" title="As Original::ID:'.$this->reference_id.', Language: '.($row->orig_language ? $row->orig_language : $row->language).'" onclick="action(\'original\',\''.$row->reference_id.'\',\''.$row->id.'\',\''.$this->reference_id.'\',\''.$row->orig_language_id.'\',\''.($row->orig_language ? $row->orig_language : $row->language).'\');">click me!</a>' : '' ) ); ?>
				</div>
				
				<div class="width-15 fltlft" style="width:15%;">
					<?php 
					echo ($row->translation_id ? $row->translation_id : ($row->trans_language ? '&nbsp;' : '<a class="hasTip translation"  title="As Translation::ID:'.$this->reference_id.', Language: '.$lang->lang_code.'" onclick="action(\'translation\',\''.$row->translation_id.'\',\''.$row->id.'\',\''.$this->reference_id.'\',\''.$this->language_id.'\',\''.$this->language.'\');">click me!</a>')); 
//					echo ($row->translation_id ? ($row->translation_id.( ($row->trans_language ) ? '&nbsp;' : ' <a class="hasTip translation" title="As Translation::ID:'.$this->reference_id.', Language: '.$lang->lang_code.'" onclick="action(\'reference\',\''.$row->translation_id.'\',\''.$row->id.'\',\''.$this->reference_id.'\',\''.$this->language_id.'\',\''.$this->language.'\');">click me!</a>') )  : ($row->trans_language ? '&nbsp;' : '<a class="hasTip translation"  title="As Translation::ID:'.$this->reference_id.', Language: '.$lang->lang_code.'" onclick="action(\'translation\',\''.$row->translation_id.'\',\''.$row->id.'\',\''.$this->reference_id.'\',\''.$this->language_id.'\',\''.$this->language.'\');">click me!</a>')); 
					?>
				</div>
...
<input type="hidden" name="task" value="translationmap.edit" />
<input type="hidden" name="tmpl" value="component" />

<input type="hidden" name="filter_order" value="<?php echo $this->order;?>" />
<input type="hidden" name="filter_order_Dir" value="<?php echo $this->orderDir;?>" />
<?php
if(count($this->requestFilters))
{
	foreach($this->requestFilters as $key => $value)
	{
		echo '<input type="hidden" name="filters['.$key.']" value="'.$value.'" />';
	}
}
